...
public class HintMasterDBHandler implements HintHandler {
    ...
    public RouteResultset route(SystemConfig sysConfig, SchemaConfig schema, int sqlType,
                                ...
                                ServerConnection sc, LayerCachePool cachePool, String hintSQLValue, int hintSqlType,
                                Map hintMap)
...
//		LOGGER.debug("hintSQLValue: " + hintSQLValue); // master/slave
        ...
        if (hintSQLValue != null && !"".equals(hintSQLValue.trim())) {
            if ("master".equalsIgnoreCase(hintSQLValue.trim())) {
                isRouteToMaster = true;
            }
            if ("slave".equalsIgnoreCase(hintSQLValue.trim())) {
...
//				}
                if (sqlType == ServerParse.DELETE || sqlType == ServerParse.INSERT
                    ...
                    LOGGER.error("should not use hint 'db_type' to route 'delete', 'insert', 'replace', 'update', " +
                            "'ddl' to a slave db.");
