...
public class DefaultDruidParser implements DruidParser {
     ...
     */
    @Override
    public void parser(SchemaConfig schema, RouteResultset rrs, SQLStatement stmt, String originSql, LayerCachePool
            cachePool, MycatSchemaStatVisitor schemaStatVisitor) throws SQLNonTransientException {
     ...
     */
    @Override
    public void visitorParse(RouteResultset rrs, SQLStatement stmt, MycatSchemaStatVisitor visitor) throws
            SQLNonTransientException {
...
//				}
    ...
    private List<RouteCalculateUnit> buildRouteCalculateUnits(SchemaStatVisitor visitor, List<List<Condition>>
            conditionList) {
        ...
        for (int i = 0; i < conditionList.size(); i++) {
            ...
            for (Condition condition : conditionList.get(i)) {
                ...
                if (checkConditionValues(values)) {
                    ...
                    if (visitor.getAliasMap() != null && visitor.getAliasMap().get(StringUtil.removeBackquote
                            (condition.getColumn().getTable().toUpperCase())) == null) {//子查询的别名条件忽略掉,不参数路由计算，否则后面找不到表
                        continue;
                    }
                    ...
                    else if ("=".equals(operator) || "in".equals(operator.toLowerCase())) { //只处理=号和in操作符,其他忽略
