...
//        getAliasMap().put("DUAL", null);
    ...
    public boolean visit(SQLBetweenExpr x) {
        ...
        for (Condition item : this.getConditions()) {
            if (item.getColumn().equals(column) && "between".equals(item.getOperator())) {
                condition = item;
                break;
            }
        }
...
//            }
    ...
    /*
     * (non-Javadoc)
     * @see com.alibaba.druid.sql.visitor.SchemaStatVisitor#visit(com.alibaba.druid.sql.ast.statement
            ...
            switch (parentExpr.getOperator()) {
                ...
                case NotLessThan:
                    ...
                    if (sexpr instanceof SQLIdentifierExpr
                            || (sexpr instanceof SQLPropertyExpr && ((SQLPropertyExpr) sexpr).getOwner() instanceof
                            SQLIdentifierExpr)) {
    ...
    public boolean visit(SQLBinaryOpExpr x) {
         ...
         *  eg. select (select id from subtest2 where id = 1), (select id from subtest3 where id = 2) from subtest1
         *  where id =4;
         *  像这样的子查询, subtest1 的 过滤条件  id = 4 .  被 加入到  subtest3 上. 加别名的情况下正常,不加别名,就会存在这个问题.
         *  这里设置好操作的是哪张表后,再进行判断.
         */
        String currenttable = x.getParent() == null ? null : (String) x.getParent().getAttribute(SchemaStatVisitor
                .ATTR_TABLE);
    ...
    private void mergeOneWhereUnit(WhereUnit whereUnit) {
        if (whereUnit.getSubWhereUnit().size() > 0) {
            ...
            if (whereUnit.getSubWhereUnit().size() > 1) {
                ...
                if (whereUnit.getOutConditions().size() > 0 && whereUnit.getSubWhereUnit().get(0).getConditionList()
                        .size() > 0) {
                    for (int i = 0; i < whereUnit.getSubWhereUnit().get(0).getConditionList().size(); i++) {
                        whereUnit.getSubWhereUnit().get(0).getConditionList().get(i).addAll(whereUnit
                                .getOutConditions());
                    }
                }
