...
public class DruidSelectDb2Parser extends DruidSelectOracleParser {
    ...
    protected void parseNativePageSql(SQLStatement stmt, RouteResultset rrs, OracleSelectQueryBlock mysqlSelectQuery,
        ...
        if (where instanceof SQLBinaryOpExpr && from instanceof SQLSubqueryTableSource) {
            ...
            if (subSelect instanceof OracleSelectQueryBlock) {
                ...
                if (hasRowNumber) {
                    if ((operator == SQLBinaryOperator.LessThan || operator == SQLBinaryOperator.LessThanOrEqual) &&
                            one.getRight() instanceof SQLIntegerExpr) {
    ...
    private static final Pattern pattern = Pattern.compile("FETCH(?:\\s)+FIRST(?:\\s)+(\\d+)(?:\\s)+ROWS(?:\\s)" +
            "+ONLY", Pattern.CASE_INSENSITIVE);

    @Override
    protected void parseNativeSql(SQLStatement stmt, RouteResultset rrs, OracleSelectQueryBlock mysqlSelectQuery,
                                  SchemaConfig schema) {
