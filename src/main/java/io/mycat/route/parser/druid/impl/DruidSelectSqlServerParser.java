...
public class DruidSelectSqlServerParser extends DruidSelectParser {
    ...
    private void parseOrderAggGroupSqlServer(SchemaConfig schema, SQLStatement stmt, RouteResultset rrs,
                                             SQLServerSelectQueryBlock mysqlSelectQuery) {
    ...
    private void parseSqlServerPageSql(SQLStatement stmt, RouteResultset rrs, SQLServerSelectQueryBlock
            sqlserverSelectQuery, SchemaConfig schema) {
        //第一层子查询
            ...
            if (subSelect instanceof SQLServerSelectQueryBlock) {
                ...
                if (subSelectOracle.getFrom() instanceof SQLSubqueryTableSource) {
                    ...
                    if (subFrom.getSelect().getQuery() instanceof SQLServerSelectQueryBlock) {
                        SQLServerSelectQueryBlock sqlSelectQuery = (SQLServerSelectQueryBlock) subFrom.getSelect()
                                .getQuery();
                ...
                if (hasRowNumber) {
                    if (hasSubTop && (operator == SQLBinaryOperator.GreaterThan || operator == SQLBinaryOperator
                            .GreaterThanOrEqual) && one.getRight() instanceof SQLIntegerExpr) {
                    ...
                    else if ((operator == SQLBinaryOperator.LessThan || operator == SQLBinaryOperator
                    ...
                    else if (operator == SQLBinaryOperator.BooleanAnd && left instanceof SQLBinaryOpExpr && one
                            .getRight() instanceof SQLBinaryOpExpr) {
                        ...
                        int firstrownum = 0;
                        int lastrownum = 0;
                        if (leftE.getRight() instanceof SQLIntegerExpr && (leftE.getOperator() == SQLBinaryOperator
                                .GreaterThan || leftE.getOperator() == SQLBinaryOperator.GreaterThanOrEqual)) {
                            small = leftE;
                        ...
                        if (rightE.getRight() instanceof SQLIntegerExpr && (rightE.getOperator() == SQLBinaryOperator
                                .GreaterThan || rightE.getOperator() == SQLBinaryOperator.GreaterThanOrEqual)) {
                            small = rightE;
