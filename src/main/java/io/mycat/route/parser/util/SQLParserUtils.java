...
public class SQLParserUtils {
    ...
    public Map<String, String> parse(String sql) {
        ...
        while (reader.hasNext()) {
            ...
            //SELECT关键字
            if ("select".equals(value) || "(select".equals(value)) {
                //if (state != null)
                state = new State();
                state.state = 0;
            ...
            //FROM关键字
            if ("from".equals(value) || "into".equals(value) || "join".equals(value)) {
                state.state = 1;
                tableFlag = true;
                continue;
            }
            //From部分出现逗号后面是表名
            if (state.state == 1 && ",".equals(value)) {
                tableFlag = true;
                continue;
            }
            ...
            if (state.state == 1 && tableFlag == false) {
                if (!value.startsWith("),") && (",".equals(value) || value.endsWith(","))) {
                    tableFlag = true;
                    continue;
                }
    ...
    private void getTableName(String str) {
        ...
        for (int i = tableFlag ? 0 : 1; i < t.length; i++) {
            if (t[i].endsWith(")")) {
                ...
                if (t[i].trim().length() > 0 && !"(".equals(t[i].trim())) {
                    tables.put(t[i], "");
                }
            }
        }
