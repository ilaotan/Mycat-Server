...
public class RouteService {
    ...
    private final String NEW_MYCAT_HINT = "/*#mycat:";    // 新的注解格式:/* !mycat: type = value */
    // sql，oldMycatHint的格式不兼容直连mysql
...
//				int hintLength = isMatchOldHint ? OLD_MYCAT_HINT.length() : NEW_MYCAT_HINT.length();
                ...
                if (firstSplitPos > 0) {
                    ...
                    if (hintSql.length() == 0) {
                        LOGGER.warn("comment int sql must meet :/*!mycat:type=value*/ or /*#mycat:type=value*/ or " +
                                "/*mycat:type=value*/: " + stmt);
                        throw new SQLSyntaxErrorException("comment int sql must meet :/*!mycat:type=value*/ or " +
                                "/*#mycat:type=value*/ or /*mycat:type=value*/: " + stmt);
                    }
                    ...
                    LOGGER.warn("comment in sql must meet :/*!mycat:type=value*/ or /*#mycat:type=value*/ or " +
                            "/*mycat:type=value*/: " + stmt);
                    throw new SQLSyntaxErrorException("comment in sql must meet :/*!mcat:type=value*/ or " +
                            "/*#mycat:type=value*/ or /*mycat:type=value*/: " + stmt);
                }
            }
        }
    ...
    private void checkMigrateRule(String schemal, RouteResultset rrs, int sqlType) throws SQLNonTransientException {
        if (rrs != null && rrs.getTables() != null) {
            ...
            ConcurrentMap<String, List<PartitionByCRC32PreSlot.Range>> tableRules = RouteCheckRule.migrateRuleMap.get
                    (schemal.toUpperCase());
            if (tableRules != null) {
                for (String table : rrs.getTables()) {
                    ...
                    if (rangeList != null && !rangeList.isEmpty()) {
                        if (rrs.getNodes().length > 1 && isUpdate) {
                            throw new SQLNonTransientException("schema:" + schemal + ",table:" + table + ",sql:" +
                                    rrs.getStatement() + " is not allowed,because table is migrate switching,please " +
                                    "wait for a moment");
                        }
                        for (PartitionByCRC32PreSlot.Range range : rangeList) {
                            ...
                            for (RouteResultsetNode routeResultsetNode : routeResultsetNodes) {
                                ...
                                if (isUpdate && slot >= range.start && slot <= range.end) {
                                    throw new SQLNonTransientException("schema:" + schemal + ",table:" + table + "," +
                                            "sql:" + rrs.getStatement() + " is not allowed,because table is migrate " +
                                            "switching,please wait for a moment");
    ...
    private boolean isUpdateSql(int type) {
        return ServerParse.INSERT == type || ServerParse.UPDATE == type || ServerParse.DELETE == type || ServerParse
                .DDL == type;
    }
