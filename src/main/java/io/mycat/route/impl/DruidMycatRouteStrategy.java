...
public class DruidMycatRouteStrategy extends AbstractRouteStrategy {
    ...
    public RouteResultset routeNormalSqlWithAST(SchemaConfig schema,
                                                ...
                                                LayerCachePool cachePool, int sqlType, ServerConnection sc) throws
            ...
            for (String tableName : tables) {
                ...
                if (index == 0) {
                    ...
                    if (tc != null) {
                        ...
                        if (firstRule != null && ((ruleCfg != null && !ruleCfg.getRuleAlgorithm().equals(firstRule
                                .getRuleAlgorithm())) || (!dataNodes.equals(firstDataNodes)))) {
                            directRoute = false;
                            break;
                        }
                    }
        ...
        if (directRoute) { //直接路由
            if (!RouterUtil.isAllGlobalTable(ctx, schemaConf)) {
                if (rulemap.size() > 1 && !checkRuleField(rulemap, visitor)) {
                    String err = "In case of slice table,sql have same rules,but the relationship condition is " +
                            "different from rule field!";
                    LOGGER.error(err);
                    throw new SQLSyntaxErrorException(err);
                }
            }
            ...
            if (subQuerySize == 0 && ctx.getTables().size() == 2) { //两表关联,考虑使用catlet
            ...
            else if (subQuerySize == 1) {     //只涉及一张表的子查询,使用  MiddlerResultHandler 获取中间结果后,改写原有 sql 继续执行 TODO
                // 后期可能会考虑多个子查询的情况.
                ...
                if (!visitor.getRelationships().isEmpty()) {     // 当 inner query  和 outer  query  有关联条件时,暂不支持
                    String err = "In case of slice table,sql have different rules,the relationship condition is not " +
                            "supported.";
                    LOGGER.error(err);
                    throw new SQLSyntaxErrorException(err);
                }
    ...
    private RouteResultset middlerResultRoute(final SchemaConfig schema, final String charset, final SQLSelect
            sqlselect,
                                              final int sqlType, final SQLStatement statement, final ServerConnection
                                                      sc) {
    ...
    private RouteResultset catletRoute(SchemaConfig schema, String stmt, String charset, ServerConnection sc) {
        ...
        try {
            rrs = MycatServer
                    ...
                    .getRouterservice()
                    .route(MycatServer.getInstance().getConfig().getSystem(),
                            schema, ServerParse.SELECT, "/*!mycat:catlet=io.mycat.catlets.ShareJoin */ " + stmt,
                            charset, sc);
    ...
    private RouteResultset directRoute(RouteResultset rrs, DruidShardingParseInfo ctx, SchemaConfig schema,
                                       DruidParser druidParser, SQLStatement statement, LayerCachePool cachePool)
            throws SQLNonTransientException {
        ...
        for (RouteCalculateUnit unit : druidParser.getCtx().getRouteCalculateUnits()) {
            RouteResultset rrsTmp = RouterUtil.tryRouteForTables(schema, druidParser.getCtx(), unit, rrs, isSelect
                    (statement), cachePool);
        ...
        for (RouteResultsetNode aNodeSet : nodeSet) {
            nodes[i] = aNodeSet;
            if (statement instanceof MySqlInsertStatement && ctx.getTables().size() == 1 && schema.getTables()
                    .containsKey(ctx.getTables().get(0))) {
    ...
    private SQLExprTableSource getDisTable(SQLTableSource tableSource, RouteResultsetNode node) throws
    ...
    private void checkUnSupportedStatement(SQLStatement statement) throws SQLSyntaxErrorException {
        ...
        if (statement instanceof MySqlReplaceStatement) {
            throw new SQLSyntaxErrorException(" ReplaceStatement can't be supported,use insert into ...on duplicate " +
                    "key update... instead ");
        }
    }
...
//									.calculateRange(pair.rangeValue.beginValue.toString(), pair.rangeValue.endValue
// .toString());
