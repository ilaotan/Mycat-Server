...
public class ShowSysParam {
    ...
    private static final String[] PARAM_DESCRIPTION = {
            "主要用于指定系统可用的线程数，默认值为Runtime.getRuntime().availableProcessors()" +
                    "方法返回的值。主要影响processorBufferPool、processorBufferLocalPercent、processorExecutor属性。NIOProcessor" +
            ...
            "主要用于指定NIOProcessor上共享的businessExecutor固定线程池大小。mycat" +
                    "在需要处理一些异步逻辑的时候会把任务提交到这个线程池中。新版本中这个连接池的使用频率不是很大了，可以设置一个较小的值。",
            "指定使用Mycat全局序列的类型。",
