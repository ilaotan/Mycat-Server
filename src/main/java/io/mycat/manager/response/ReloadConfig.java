...
public final class ReloadConfig {
    ...
    public static void execute(ManagerConnection c, final boolean loadAll) {
        ...
        if (loadAll && !NIOProcessor.backends_old.isEmpty()) {
            c.writeErrMessage(ErrorCode.ER_YES, "The are several unfinished db transactions before executing \"reload" +
                    " @@config_all\", therefore the execution is terminated for logical integrity and please try " +
                    "again later.");
            return;
        }
        ...
        try {
            ...
            Futures.addCallback(listenableFuture, new ReloadCallBack(c), MycatServer.getInstance()
