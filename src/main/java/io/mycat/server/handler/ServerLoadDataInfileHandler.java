...
public final class ServerLoadDataInfileHandler implements LoadDataInfileHandler {
    ...
    public void start(String sql) {
        ...
        tableId2DataNodeCache = (LayerCachePool) MycatServer.getInstance().getCacheService().getCachePool
        ...
        tempPath = SystemConfig.getHomePath() + File.separator + "temp" + File.separator + serverConnection.getId() +
                File.separator;
    ...
    private RouteResultset tryDirectRoute(String sql, String[] lineList) {
        ...
        if (tableConfig == null && schema.getDataNode() != null) {
            ...
            if (partitionColumnIndex == -1 || partitionColumnIndex >= lineList.length) {
                ...
                routeCalculateUnit.addShardingExpr(tableName, getPartitionColumn(), parseFieldString(value, loadData
                        .getEnclose()));
                ctx.addRouteCalculateUnit(routeCalculateUnit);
                try {
                    SortedSet<RouteResultsetNode> nodeSet = new TreeSet<RouteResultsetNode>();
                    for (RouteCalculateUnit unit : ctx.getRouteCalculateUnits()) {
                        RouteResultset rrsTmp = RouterUtil.tryRouteForTables(schema, ctx, unit, rrs, false,
                                tableId2DataNodeCache);
                        if (rrsTmp != null) {
    ...
    private String joinField(String[] src, LoadData loadData) {
        ...
        for (int i = 0, srcLength = src.length; i < srcLength; i++) {
            ...
            if (loadData.getEnclose() == null) {
                ...
                sb.append(loadData.getEnclose()).append(s.replace(loadData.getEnclose(), loadData.getEscape() +
                        loadData.getEnclose())).append(loadData.getEnclose());
            }
            if (i != srcLength - 1) {
