...
public final class MigrateHandler {
    ...
    //可以优化成多个锁
    private static InterProcessMutex slaveIDsLock = new InterProcessMutex(ZKUtils.getConnection(), ZKUtils
            .getZKBasePath() + "lock/slaveIDs.lock");
    ...
    public static void handle(String stmt, ServerConnection c) {
        ...
        try {
            ...
            Map<String, List<MigrateTask>> tasks = MigrateUtils
                    .balanceExpand(table, integerListMap, oldDataNodes, newDataNodes, PartitionByCRC32PreSlot
                            .DEFAULT_SLOTS_NUM);
            ...
            if (client.checkExists().forPath(taskBase) != null) {
                ...
                for (String child : childTaskList) {
                    TaskNode taskNode = JSON
                            .parseObject(ZKUtils.getConnection().getData().forPath(taskBase + "/" + child), TaskNode
                                    .class);
                    if (taskNode.getSchema().equalsIgnoreCase(c.getSchema()) && table.equalsIgnoreCase(taskNode
                            .getTable())
                            && taskNode.getStatus() < 5) {
                        writeErrMessage(c, "table: " + table + " previous migrate task is still running,on the same " +
                                "time one table only one task");
                        return;
                    }
                }
            }
    ...
    private static int getSlaveIdFromZKForDataNode(String dataNode) {
        ...
        if (Strings.isNullOrEmpty(slaveIDs)) {
            throw new RuntimeException("dataHost:" + dbNode.getDbPool().getHostName() + " do not config the salveIDs " +
                    "field");
        }
