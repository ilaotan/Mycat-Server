...
public class ExplainHandler {
    ...
    private final static Pattern              pattern     = Pattern.compile("(?:" +
            "(\\s*next\\s+value\\s+for\\s*MYCATSEQ_(\\w+))(,|\\)|\\s)*)+", Pattern.CASE_INSENSITIVE);
    ...
    private static RouteResultset getRouteResultset(ServerConnection c,
        ...
        try {
            ...
            if (ServerParse.INSERT == sqlType && isMycatSeq(stmt, schema)) {
                c.writeErrMessage(ErrorCode.ER_PARSE_ERROR, "insert sql using mycat seq,you must provide primaryKey " +
                        "value for explain");
                return null;
            }
