...
public class SchemaUtil {
    ...
    public static String detectDefaultDb(String sql, int type) {
        ...
        else if ((ServerParse.SHOW == type || ServerParse.USE == type || ServerParse.EXPLAIN == type || ServerParse
                .SET == type
    ...
    private static Pattern pattern = Pattern.compile("^\\s*(SHOW)\\s+(FULL)*\\s*(TABLES)\\s+(FROM)\\s+([a-zA-Z_0-9]+)" +
            "\\s*([a-zA-Z_0-9\\s]*)", Pattern.CASE_INSENSITIVE);
