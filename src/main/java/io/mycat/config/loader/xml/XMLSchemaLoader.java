...
public class XMLSchemaLoader implements SchemaLoader {
    ...
    private String doTableNameSuffix(String tableNameElement, String tableNameSuffixElement) {
        ...
        else if ("YYYYMMDD".equals(suffixFormat)) {
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            if (!"".equals(tableNameSuffixElement)) {
                ...
                if (tableNameElement.split(",").length > 1) {
                    throw new ConfigException("nameSuffix " + tableNameSuffixElement + ", require name parameter " +
                            "cannot multiple breaks!");
                }
                //前缀用来标明日期格式
            ...
            String primaryKey = tableElement.hasAttribute("primaryKey") ? tableElement.getAttribute("primaryKey")
                    .toUpperCase() : null;
    ...
    private void processChildTables(Map<String, TableConfig> tables,
        ...
        for (int j = 0; j < childNodeList.getLength(); j++) {
            ...
            if (!"childTable".equals(theNode.getNodeName())) {
                continue;
            }
            Element childTbElement = (Element) theNode;
            //读取子表信息
            String cdTbName = childTbElement.getAttribute("name").toUpperCase();
            String primaryKey = childTbElement.hasAttribute("primaryKey") ? childTbElement.getAttribute("primaryKey")
                    .toUpperCase() : null;
        ...
        switch (suitValue) {
            case -1:
                ...
                throw new ConfigException("Illegal table conf : table [ " + tableConf.getName() + " ] rule function [ "
                        ...
                        + tableConf.getDataNodes().size() + ", please make sure table datanode size = function " +
                        "partition size");
            case 0:
            ...
            case 1:
                // 有些节点是多余的,给出warn log
                LOGGER.warn("table conf : table [ {} ] rule function [ {} ] partition size : {} < table datanode size" +
    ...
    private void loadDataHosts(Element root) {
        ...
        for (int i = 0, n = list.getLength(); i < n; ++i) {
            ...
            int switchType = "".equals(switchTypeStr) ? -1 : Integer.parseInt(switchTypeStr);
            //读取从延迟界限
            String slaveThresholdStr = element.getAttribute("slaveThreshold");
            int slaveThreshold = "".equals(slaveThresholdStr) ? -1 : Integer.parseInt(slaveThresholdStr);
            ...
            boolean tempReadHostAvailable = !"".equals(tempReadHostAvailableStr) && Integer.parseInt(tempReadHostAvailableStr) > 0;
            /**
             * 读取 写类型
