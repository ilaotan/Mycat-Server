...
public class MysqlDataIO implements DataIO {
    ...
    @Override
    public void importData(TableMigrateInfo table, DataNode dn, String tableName, File file) throws IOException,
...
//		String loadData ="?mysql -h? -P? -u? -p? -D? --local-infile=1 -e \"load data local infile '?' replace into
// table ? CHARACTER SET '?' FIELDS TERMINATED BY ','  LINES TERMINATED BY '\\r\\n'\"";
        ...
        loadData = DataMigratorUtil.paramsAssignment(loadData, "?", mysqlBin, ip, port, user, pwd, db, charset, file
                .getAbsolutePath());
    ...
    @Override
    public File exportData(TableMigrateInfo table, DataNode dn, String tableName, File export, File condition) throws
...
// (?)'";
        ...
        String mysqlDump = "?mysqldump -h? -P? -u? -p? ? ?  --compact --no-create-info --default-character-set=? " +
                "--add-locks=false --where=\"? in (#)\" --result-file=\"?\"";
        ...
        mysqlDump = DataMigratorUtil.paramsAssignment(mysqlDump, "?", mysqlBin, ip, port, user, pwd, db, tableName,
                charset, table.getColumn(), exportFile);
