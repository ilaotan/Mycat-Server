...
public class DataMigrator {
    ...
    public static void main(String[] args) throws SQLException {
        ...
        System.out.println("\n" + format.format(new Date()) + " [1]-> creating migrator schedule and temp files for " +
                "migrate...");
        //初始化配置
        ...
        for (TableMigrateInfo table : migrateTables) {
            if (!table.isError()) { //忽略已出错的拆分表
                ...
                for (DataNodeMigrateInfo info : detailList) {
                    executor.execute(new DataMigrateRunner(table, info.getSrc(), info.getTarget(), table.getTableName
                            (), info.getTempFile()));
                }
            }
        }
