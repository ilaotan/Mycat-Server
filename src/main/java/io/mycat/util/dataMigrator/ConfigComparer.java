...
public class ConfigComparer {
    ...
    private void loadNewConfig() {
        try {
            ...
            throw new ConfigException(" new config for migrate read fail!please check newSchema.xml or  newRule.xml  " +
                    "" + e);
        }
    ...
    private String[] getTables(String tables) {
        if ("all".equalsIgnoreCase(tables) || tables.isEmpty()) {
            return new String[]{};
        }
        else {
            return tables.split(",");
    ...
    private void loadMigratorTable(TableConfig oldTable, TableConfig newTable, String schemaName, String tableName) {
        ...
        if (oldTable == null || newTable == null) {
            throw new ConfigException("please check tableFile.properties,make sure " + schemaName + ":" + tableName +
                    " is sharding table ");
        }
        //忽略全局表
        if (oldTable.isGlobalTable() || newTable.isGlobalTable()) {
            ...
            if (isNeedMigrate(oldDN, newDN)) {
                ...
                TableMigrateInfo tmi = new TableMigrateInfo(schemaName, tableName, oldDN, newDN, newRC
                        .getRuleAlgorithm(), newRC.getColumn());
    ...
    private List<DataNode> getDataNodes(TableConfig tableConfig, Map<String, DataNodeConfig> dnConfig, Map<String,
            DataHostConfig> dhConfig) {
