...
public abstract class AbstractConnection implements NIOConnection {
    ...
    protected final    ConcurrentLinkedQueue<byte[]> decompressUnfinishedDataQueue = new
    ...
    public boolean setCharset(String charset) {
        ...
        if (ci > 0) {
            this.charset = "utf8mb4".equalsIgnoreCase(charset) ? "utf8" : charset;
                ...
                if (position == offset) {
                    ...
                    if (readBuffer != null && !readBuffer.isDirect()
                        ...
                        if (LOGGER.isDebugEnabled()) {
                            LOGGER.debug("change to direct con read buffer ,cur temp buf size :" + readBuffer
                                    .capacity());
                        }
                        recycle(readBuffer);
                        readBuffer = processor.getBufferPool().allocate(processor.getBufferPool()
