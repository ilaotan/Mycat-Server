...
public class MultiNodeCoordinator implements ResponseHandler {
    ...
    public void executeBatchNodeCmd(SQLCtrlCommand cmdHandler) {
        ...
        for (RouteResultsetNode rrn : session.getTargetKeys()) {
            ...
            if (conn != null) {
                ...
                if (mysqlCon.getXaStatus() == TxState.TX_STARTED_STATE) {
                    //recovery Log
                    participantLogEntry[started] = new ParticipantLogEntry(xaTxId, conn.getHost(), 0, conn.getSchema
                            (), ((MySQLConnection) conn).getXaStatus());
        ...
        //xa recovery log
        if (session.getXaTXID() != null) {
            CoordinatorLogEntry coordinatorLogEntry = new CoordinatorLogEntry(session.getXaTXID(), false,
                    participantLogEntry);
