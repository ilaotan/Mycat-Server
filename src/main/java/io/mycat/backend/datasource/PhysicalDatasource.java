...
public abstract class PhysicalDatasource {
    ...
    private              ConcurrentMap<Long /* ConnectionId */, String /* 常量1*/> takeConnectionContext = new
            ConcurrentHashMap<>();
...
//		int ildeCloseCount = hostConfig.getMinCon() * 3;
    ...
    /**
     * @param ildeCloseCount 首先，从已创建的连接中选择本次心跳需要关闭的空闲连接数（由当前连接连接数-减去配置的最小连接数。
            ...
            try {
                ...
                while (curTotalConnection + 1 <= size) {
                    ...
                    if (this.totalConnection.compareAndSet(curTotalConnection, curTotalConnection + 1)) {
                        ...
                        LOGGER.info("no ilde connection in pool,create new connection for " + this.name + " of schema" +
                                " " + schema);
