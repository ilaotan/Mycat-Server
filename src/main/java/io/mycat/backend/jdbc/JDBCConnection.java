...
public class JDBCConnection implements BackendConnection {
    ...
    public void setDbType(String newDbType) {
        ...
        this.isSpark = "SPARK".equals(dbType);
    ...
    private void executeSQL(RouteResultsetNode rrn, ServerConnection sc,
        ...
        try {
            ...
            if (rrn.isCallStatement() && "oracle".equalsIgnoreCase(getDbType())) {
                ...
                if ((sqlType == ServerParse.SHOW) && (!"MYSQL".equals(dbType))) {
                    // showCMD(sc, orgin);
        ...
        try {
            ...
            for (ProcedureParameter paramter : paramters) {
                ...
                if (ProcedureParameter.OUT.equalsIgnoreCase(paramter.getParameterType())
                        || ProcedureParameter.INOUT.equalsIgnoreCase(paramter.getParameterType())) {
                    int jdbcType = "oracle".equalsIgnoreCase(getDbType()) && procedure.getListFields().contains
                            (paramter.getName()) ? oracleCURSORTypeValue : paramter.getJdbcType();
            ...
            if (procedure.getSelectColumns().size() > 0) {
                ...
                for (ProcedureParameter paramter : paramters) {
                    if (!procedure.getListFields().contains(paramter.getName()) && (ProcedureParameter.OUT
                            .equalsIgnoreCase(paramter.getParameterType())
                            || ProcedureParameter.INOUT.equalsIgnoreCase(paramter.getParameterType()))) {
                        FieldPacket packet = PacketUtil.getField(paramter.getName(), MysqlDefs.javaTypeMysql(paramter
                                .getJdbcType()));
                        fieldPks.add(packet);
                    }
                }
                ...
                for (int i = 0; i < colunmCount; i++) {
                    ...
                    if (MysqlDefs.isBianry((byte) fieldPks.get(i).type)) {
                        curRow.add(rs.getBytes(j));
                            ...
                            fieldPks.get(i).type == (MysqlDefs.FIELD_TYPE_NEW_DECIMAL - 256)) { // field type is
                        // unsigned byte
