...
public class BinlogStream {
    ...
    public void setMigrateTaskList(List<MigrateTask> migrateTaskList) {
        ...
        for (MigrateTask migrateTask : migrateTaskList) {
            ...
            if (!semaphoreMap.containsKey(dataHostTo)) {
                int count = Double.valueOf(MycatServer.getInstance().getConfig().getDataHosts().get(dataHostTo)
                        .getSource().getSize() * 0.8).intValue();
    ...
    private final class DelegatingEventListener implements BinaryLogClient.EventListener {
        ...
        private final Map<Long, TableMapEventData>                   tablesById      = new HashMap<Long,
                TableMapEventData>();
        ...
        private Map<Integer, Map<String, Object>> loadColumn(String database, String table) {
            ...
            try {
                ...
                list = executeQuery(con, "select  COLUMN_NAME, ORDINAL_POSITION, DATA_TYPE, CHARACTER_SET_NAME from " +
                        "INFORMATION_SCHEMA.COLUMNS where table_name='" + table + "' and TABLE_SCHEMA='" + database +
                        "'");
        ...
        public void onEvent(Event event) {
            ...
            switch (eventType) {
                case TABLE_MAP:
                    ...
                    if (!tablesColumnMap.containsKey(tableMapEventData.getDatabase() + "." + tableMapEventData
                            .getTable())) {
                        tablesColumnMap.put(tableMapEventData.getDatabase() + "." + tableMapEventData.getTable(),
                                loadColumn(tableMapEventData.getDatabase(), tableMapEventData.getTable()));
                    }
                    break;
                case ROTATE:
        ...
        private void handleOtherSqlEvent(Event event) {
            ...
            for (MigrateTask migrateTask : migrateTaskList) {
                if (schemaInfo.table.equalsIgnoreCase(migrateTask.getTable())
                        && queryEventData.getDatabase().equalsIgnoreCase(MigrateUtils.getDatabaseFromDataNode
                        (migrateTask.getFrom()))) {
        ...
        private boolean isShouldBeFilter(String database, String table) {
            ...
            for (MigrateTask migrateTask : migrateTaskList) {
                if (database.equals(MigrateUtils.getDatabaseFromDataNode(migrateTask.getFrom())) && table
                        .equalsIgnoreCase(migrateTask.getTable())) {
                    return false;
                }
            }
        ...
        private void handleWriteRowsEvent(Event event) {
            ...
            Map<Integer, Map<String, Object>> xxx = tablesColumnMap.get(tableMapEvent.getDatabase() + "." +
            ...
            for (int i = 0; i < rows.size(); i++) {
                ...
                for (int y = 0; y < size; y++) {
                    ...
                    if ("_slot".equalsIgnoreCase(columnName)) {
                        slot = value[y] instanceof BigInteger ? ((BigInteger) value[y]).intValue() : ((Integer)
                                value[y]);
                    }
        ...
        private void checkIfExeSql(TableMapEventData tableMapEvent, StringBuilder sb, int slot) {
            for (MigrateTask migrateTask : migrateTaskList) {
                if (tableMapEvent.getTable().equalsIgnoreCase(migrateTask.getTable())
                        && tableMapEvent.getDatabase().equalsIgnoreCase(MigrateUtils.getDatabaseFromDataNode
                        (migrateTask.getFrom()))) {
