...
public class MigrateDumpRunner implements Runnable {
    ...
    public void run() {
        try {
            String mysqldump = "?mysqldump -h? -P? -u? -p?  ? ? --single-transaction -q " +
                    "--default-character-set=utf8mb4 --hex-blob --where=\"?\" --master-data=1  -T  \"?\"  " +
                    "--fields-enclosed-by=\\\" --fields-terminated-by=, --lines-terminated-by=\\n  " +
                    "--fields-escaped-by=\\\\ ";
            PhysicalDBPool dbPool = MycatServer.getInstance().getConfig().getDataNodes().get(task.getFrom())
                    .getDbPool();
            ...
            String finalCmd = DataMigratorUtil
                    .paramsAssignment(mysqldump, "?", "", config.getIp(), String.valueOf(config.getPort()), config
                                    .getUser(),
                            config.getPassword(), MigrateUtils.getDatabaseFromDataNode(task.getFrom()), task.getTable
                                    (), makeWhere(task), file.getPath());
            List<String> args = Arrays.asList("mysqldump", "-h" + config.getIp(), "-P" + String.valueOf(config
                            .getPort()), "-u" + config.getUser(),
                    "-p" + config.getPassword(), MigrateUtils.getDatabaseFromDataNode(task.getFrom()), task.getTable
                            (), "--single-transaction", "-q", "--default-character-set=utf8mb4", "--hex-blob",
                    "--where=" + makeWhere(task), "--master-data=1", "-T" + file.getPath()

                    , "--fields-enclosed-by=" + encose + "\"", "--fields-terminated-by=,",
                    "--lines-terminated-by=\\n", "--fields-escaped-by=\\\\");
            ...
            String logFile = result.substring(logIndex + 17, logIndex + 17 + result.substring(logIndex + 17).indexOf
                    ("'"));
            String logPos = result.substring(logPosIndex + 15, logPosIndex + 15 + result.substring(logPosIndex + 15)
                    .indexOf(";"));
    ...
    private String extractCreateSql(List<String> lines) {
        ...
        for (String line : lines) {
            if (Strings.isNullOrEmpty(line) || line.startsWith("--") || line.startsWith("/*") || line.startsWith
                    ("DROP")) {
                isAdd = false;
                continue;
            }
    ...
    private void exeCreateTableToDn(String sql, String toDn, String table) throws SQLException {
        ...
        Connection con = null;
        try {
            con = DriverManager.getConnection("jdbc:mysql://" + config.getUrl() + "/" + dbNode.getDatabase(), config
    ...
    private void pushMsgToZK(String rootZkPath, String child, int status, String msg, String binlogFile, String pos)
            throws Exception {
    ...
    private void loaddataToDn(File loaddataFile, String toDn, String table) throws SQLException, IOException {
        ...
        try {
            con = DriverManager.getConnection("jdbc:mysql://" + config.getUrl() + "/" + dbNode.getDatabase(), config
                    .getUser(), config.getPassword());
            String sql = "load data local infile '" + loaddataFile.getPath().replace("\\", "//") + "' replace into " +
                    "table " + table + " character set 'utf8mb4'  fields terminated by ','  enclosed by '\"'  ESCAPED" +
