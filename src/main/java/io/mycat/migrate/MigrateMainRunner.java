...
public class MigrateMainRunner implements Runnable {
    ...
    public void run() {
        ...
        for (MigrateTask migrateTask : migrateTaskList) {
            MycatServer.getInstance().getBusinessExecutor().submit(new MigrateDumpRunner(migrateTask, downLatch,
                    sucessTask));
        }
        try {
        ...
        if (sucessTask.get() == migrateTaskList.size()) {
            ...
            for (MigrateTask migrateTask : migrateTaskList) {
                if (binlogFileNum == -1) {
                    binlogFileNum = Integer.parseInt(migrateTask.getBinlogFile().substring(migrateTask.getBinlogFile
                            ().lastIndexOf(".") + 1));
                    binlogFile = migrateTask.getBinlogFile();
                    pos = migrateTask.getPos();
                }
                else {
                    int tempBinlogFileNum = Integer.parseInt(migrateTask.getBinlogFile().substring(migrateTask
                            .getBinlogFile().lastIndexOf(".") + 1));
            ...
            BinlogStream stream = new BinlogStream(config.getUrl().substring(0, config.getUrl().indexOf(":")), config
                    .getPort(), config.getUser(), config.getPassword());
            try {
