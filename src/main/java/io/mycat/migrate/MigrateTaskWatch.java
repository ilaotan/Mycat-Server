                ...
                if (!dataNodeList.isEmpty()) {
                    if ((!Strings.isNullOrEmpty(text)) && text.startsWith("{")) {
                        ...
                        if (taskNode.getStatus() == 0) {
                            String boosterDataHosts = ZkConfig.getInstance().getValue(ZkParamCfg
                                    .MYCAT_BOOSTER_DATAHOSTS);
                            Set<String> dataNodes = new HashSet<>(Splitter.on(",").trimResults().omitEmptyStrings()
                                    .splitToList(boosterDataHosts));
                            ...
                            for (String s : dataNodeList) {
                                ...
                                if (dataNodes.contains(s)) {
                                    ...
                                    String data = new String(ZKUtils.getConnection().getData().forPath(zkpath),
                                            "UTF-8");
                            ...
                            for (Map.Entry<String, List<MigrateTask>> stringListEntry : taskMap.entrySet()) {
                                ...
                                MycatServer.getInstance().getBusinessExecutor().submit(new MigrateMainRunner(key,
                                        value));
                            }

                            //
                            taskNode.setStatus(1);
                            ZKUtils.getConnection().setData().forPath(event.getData().getPath(), JSON.toJSONBytes
                                    (taskNode));
                        }
                        else if (taskNode.getStatus() == 2) {
                            //start switch

                            ScheduledExecutorService scheduledExecutorService = MycatServer.getInstance()
                                    .getScheduler();
                            ...
                            if (!allRunnerSet.contains(taskID)) {
                                ...
                                List<MigrateTask> allTaskList = MigrateUtils.queryAllTask(tpath, removeStatusNode
                                        (dataHosts));
