...
public class MigrateUtils {
     ...
     * @return
     */
    public static Map<String, List<MigrateTask>> balanceExpand(String table, Map<Integer, List<Range>>
            integerListMap, List<String> oldDataNodes,
    ...
    public static List<MigrateTask> queryAllTask(String basePath, List<String> dataHost) throws Exception {
        ...
        for (String dataHostName : dataHost) {
            ...
            resutlList.addAll(JSON
                    .parseArray(new String(ZKUtils.getConnection().getData().forPath(basePath + "/" + dataHostName),
                            "UTF-8"), MigrateTask.class));
        }
        return resutlList;
    }
    ...
    public static void execulteSql(String sql, String toDn) throws SQLException, IOException {
        ...
        try {
            con = DriverManager
                    .getConnection("jdbc:mysql://" + config.getUrl() + "/" + dbNode.getDatabase(), config.getUser(),
                            config.getPassword());
