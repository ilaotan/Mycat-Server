...
public class ShareJoin implements Catlet {
    ...
    public void route(SystemConfig sysConfig, SchemaConfig schema, int sqlType, String realSQL, String charset,
        ...
        try {
            ...
            // rrs =RouteStrategyFactory.getRouteStrategy().route(sysConfig, schema, sqlType2, realSQL,charset, sc,
            // cachePool);
    ...
    private void getRoute(String sql) {
        try {
            if (joinParser != null) {
                rrs = RouteStrategyFactory.getRouteStrategy().route(sysConfig, schema, sqltype, sql, charset, sc,
                        cachePool);
            }
        }
        catch (Exception e) {
...
class ShareRowOutPutDataHandler implements SQLJobHandler {
    ...
    public ShareRowOutPutDataHandler(ShareJoin ctx, List<byte[]> afields, int joini, String joinField, Map<String,
            byte[]> arows, NonBlockingSession session) {
        super();
