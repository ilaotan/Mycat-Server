...
public class BaseSQLExeTest {
    ...
    private static void testMultiNodeNormalSQL(Connection theCon)
        ...
        String[] sqls = {
                ...
                "select sum(fee) total_fee, days,count(id),max(fee),min(fee) from  travelrecord  group by days  order" +
                        " by days desc limit 99 ",
    ...
    private static void testBadSQL(Connection theCon) throws SQLException {
        ...
        theCon.setAutoCommit(true);
        String[] sqls = {
                "select sum(fee) total_fee, days,count(id),max(fee),min(fee) from  travelrecord  group by id  order " +
                        "by id desc limit 99",
                ...
                "insert into travelrecord (id,user_id,traveldate,fee,days) values(1,’wang’,’2014-01-05’,510.5,3)",
                "insert into travelrecord (id,user_id,traveldate,fee,days,badcolumn) values(1,’wang’,’2014-01-05’," +
                        "510.5,3)",
                "select sum(fee) total_fee, days,count(id),max(fee),min(fee) from  travelrecord  group by count(id)  " +
                        "order by count(id) desc limit 99 "};
        for (String sql : sqls) {
            try {
                System.out.println("execute " + sql);
